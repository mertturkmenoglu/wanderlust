...
function RouteComponent() {
	...
	const rows = defineRows([
		...
		['Phone', place.intlPhone ?? '—'],
		['Website', renderer.ExternalLink('Website', place.websites[0] ?? '—')],
		// [
		// 	'Address',
		// 	renderer.Link(
		// 		'Address',
		...
		[
			...
			renderer.Link(
				'Category',
				{
					...
					params: {
						id: place.primaryCategoryId,
					},
				},
				<CategoryCard className="min-w-sm" category={place.primaryCategory} />,
			),
		],
		...
		[
			...
			<KeyValueList
				variant="bordered"
				items={[
					{ label: 'Pre', value: JSON.stringify(place.openingHours, null, 2) },
				]}
			/>,
		],
			...
			renderer.Multiple(
				place.accolades.map((a) =>
					renderer.Link(
						...
						<AccoladeCard accolade={a} className="min-w-sm" />,
					),
				),
			),
		],
