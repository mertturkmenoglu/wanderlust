...
export const wikiVoyage = command({
	name: 'voyage',
	desc: 'Wikivoyage City info extraction',
	options: {
		name: string('name').desc('The name of the city to fetch information for.'),
		url: string('url').desc(
			'The URL to fetch the Wikivoyage page from. If provided, the name option will be ignored.',
		),
		view: string('view')
			.enum('basic', 'full')
			.default('basic')
	...
	handler: async (opts) => {
		...
		if (opts.url) {
			...
			url = `https://en.wikivoyage.org/w/api.php?action=query&prop=extracts&explaintext=1&titles=${formattedName}&format=json`;
		} else {
