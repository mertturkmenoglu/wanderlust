...
- When user submits the review, we send the plain text content to the backend. The backend uses `@wanderlust/richtext` to extract the facets from the review content and store them in the database.
