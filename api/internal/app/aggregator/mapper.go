...
func mapRow(row Row) HomeAggregatorPoi {
	return HomeAggregatorPoi{
		...
		FirstMedia: HomeAggregatorPoiMedia{
			...
			Url:        row.Medium.Url,
			Thumbnail:  row.Medium.Thumbnail,
			Alt:        row.Medium.Alt,
			Caption:    utils.TextOrNil(row.Medium.Caption),
			Width:      row.Medium.Width,
			Height:     row.Medium.Height,
			MediaOrder: row.Medium.MediaOrder,
			Extension:  row.Medium.Extension,
			MimeType:   row.Medium.MimeType,
			FileSize:   row.Medium.FileSize,
